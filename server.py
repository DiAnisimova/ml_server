...
class InfoForm(FlaskForm):
    model_number = IntegerField('Номер модели', validators=[validators.NumberRange(min=1, max=1000)])
...
def prepare_model():
    ...
    if request.method == 'POST' and cur_model.validate_on_submit():
        ...
        if cur_model.target.data not in X.columns:
            ...
            error = True
        elif cur_model.model_type.data == 'GradientBoosting' and cur_model.learning_rate.data == 0:
        ...
        if not error:
            ...
            if cur_model.feature_subsample_size.data == 0:
                ...
                fss = cur_model.feature_subsample_size.data
            if cur_model.max_depth.data == 0:
                max_depth = None
            else:
                max_depth = cur_model.max_depth.data
...
def model_information():
    ...
    if request.method == 'POST' and info.validate_on_submit():
        if info.model_number.data > len(models):
            flash("Такой модели пока не существует!")
            return redirect(url_for('model_information'))
